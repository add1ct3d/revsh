...
/* The decision below to use gethostbyname() instead of getaddrinfo() is a
 * design decision. */
/* If the application is being built without SSL, then the target host is
 * probably a much older host with */
/* equally old network libraries. The "no SSL" case would better be considered
 * here as the "deprecated best */
...
int init_io_control() {
  ...
  /* In the no ssl build, there is no difference between a control in bindshell
   * mode, and a target. */
  ...
  if ((ip_address = calloc(ip_address_len + 1, sizeof(char))) == NULL) {
    report_error("init_io_control(): calloc(%d, %d): %s", ip_address_len,
  ...
  if ((ip_port = strchr(ip_address, ':')) == NULL) {
    report_error("init_io_control(): strchr(%s, ':'): Port not found!",
                 ip_address);
    return (-1);
  }
  *ip_port = '\0';
  ...
  if ((host = gethostbyname(ip_address)) == NULL) {
    report_error("init_io_control(): gethostbyname(%s): %s", ip_address,
                 strerror(errno));
    return (-1);
  }
  ...
  if ((tmp_sock = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    report_error("init_io_control(): socket(AF_INET, SOCK_STREAM, 0): %s",
                 strerror(errno));
    return (-1);
  }
  ...
  if (setsockopt(tmp_sock, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) == -1) {
    report_error(
        "init_io_control(): setsockopt(%d, SOL_SOCKET, SO_REUSEADDR, %lx, %d): "
        "%s",
  ...
  if (sigaction(SIGALRM, &act, NULL) == -1) {
    report_error("init_io_control(): sigaction(%d, %lx, %p): %s", SIGALRM,
  ...
  if (bind(tmp_sock, (struct sockaddr *)&name, sizeof(name)) == -1) {
    report_error("init_io_control(): bind(%d, %lx, %d): %s", tmp_sock,
                 (unsigned long)&name, (int)sizeof(name), strerror(errno));
    return (-1);
  }

  if (listen(tmp_sock, 1) == -1) {
    report_error("init_io_control(): listen(%d, 1): %s", tmp_sock,
                 strerror(errno));
    return (-1);
  }

  if ((io->remote_fd = accept(tmp_sock, NULL, NULL)) == -1) {
    report_error("init_io_control(): accept(%d, NULL, NULL): %s", tmp_sock,
                 strerror(errno));
    return (-1);
  }
...
int init_io_target() {
  ...
  /* In the no ssl build, there is no difference between a target in bindshell
   * mode, and the control node for networking. */
  /* As such, we'll just pass through to the other rather than repeat code. */
  if (io->target && config->bindshell) {
  ...
  ip_address_len = strlen(config->ip_addr);
  if ((ip_address = calloc(ip_address_len + 1, sizeof(char))) == NULL) {
    report_error("init_io_target(): calloc(%d, %d): %s", ip_address_len,
                 (int)sizeof(char), strerror(errno));
    return (-1);
  }
  ...
  if ((ip_port = strchr(ip_address, ':')) == NULL) {
    report_error("init_io_target(): strchr(%s, ':'): Port not found!",
                 ip_address);
    return (-1);
  }
  *ip_port = '\0';
  ...
  if ((host = gethostbyname(ip_address)) == NULL) {
    report_error("init_io_target(): gethostbyname(%s): %s", ip_address,
                 strerror(errno));
    return (-1);
  }
  ...
  if ((tmp_sock = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    report_error("init_io_target(): socket(AF_INET, SOCK_STREAM, 0): %s",
                 strerror(errno));
    return (-1);
  }
  ...
  while ((retval = connect(tmp_sock, (struct sockaddr *)&name, sizeof(name)))) {
    if (retval == -1) {
      report_error("init_io_target(): connect(%d, %lx, %d): %s", io->remote_fd,
                   (unsigned long)&name, (int)sizeof(name), strerror(errno));
