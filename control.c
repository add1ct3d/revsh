...
int do_control() {
  ...
  // -1 is the only fatal error here. -2 and -3 are also failures, but should
  // not be fatal.
  if ((retval = init_io_control(config)) == -1) {
    report_error("do_control(): init_io_control(%lx): %s",
                 (unsigned long)config, strerror(errno));
    return (-1);
  }

  // retval == -2  means control in bindshell + keepalive mode and we need to
  // return to handle another connection.
  if (retval == -2) {
    io->init_complete = 1;
    return (-2);
  }

  // retval == -3 means the SSL handshake failed for some reason. We should
  // reset.
  if (retval == -3) {
    io->init_complete = 0;
    return (-3);
  ...
  /* Start conversing with the remote partner to agree on the shape of this
   * session. */
  ...
  /* Both sides must agree on interaction. If either one opts out, fall back to
  ...
  if (wordexp(config->rc_file, &rc_file_exp, 0)) {
    report_error("do_control(): wordexp(%s, %lx, 0): %s", config->rc_file,
                 (unsigned long)&rc_file_exp, strerror(errno));
    return (-1);
  }
  ...
  /* If the C2 hasn't been invoked with a specific shell, then let the client
   * choose. */
  ...
  if ((exec_envp = string_to_vector(DEFAULT_ENV)) == NULL) {
    report_error("do_control(): string_to_vector(%s): %s", DEFAULT_ENV,
                 strerror(errno));
    return (-1);
  }
  ...
  for (i = 0; exec_envp[i]; i++) {
    /* This will be the length of the env variable name, plus one char for '='
     * and one char for ' ' or '\0'. */
  ...
  for (i = 0; exec_envp[i]; i++) {
    ...
    *(message->data + message->data_len++) = '=';
    ...
    if ((tmp_ptr = getenv(exec_envp[i])) == NULL) {
      report_error(
          "do_control(): No such environment variable \"%s\". Ignoring.",
          exec_envp[i]);
    } else {
      io_bytes = strlen(tmp_ptr);
  ...
  if (ioctl(STDIN_FILENO, TIOCGWINSZ, &tty_winsize) == -1) {
    report_error("do_control(): ioctl(STDIN_FILENO, TIOCGWINSZ, %lx): %s",
  ...
  *((unsigned short *)(message->data + message->data_len)) =
  ...
  if ((io->saved_termios_attrs =
           (struct termios *)calloc(1, sizeof(struct termios))) == NULL) {
    report_error("do_control(): calloc(1, %d): %s", (int)sizeof(struct termios),
                 strerror(errno));
    return (-1);
  }

  if ((io->revsh_termios_attrs =
           (struct termios *)calloc(1, sizeof(struct termios))) == NULL) {
    report_error("do_control(): calloc(1, %d): %s", (int)sizeof(struct termios),
                 strerror(errno));
    return (-1);
  ...
  if (tcgetattr(STDIN_FILENO, io->saved_termios_attrs) == -1) {
    report_error("do_control(): tcgetattr(STDIN_FILENO, %lx): %s",
                 (unsigned long)io->saved_termios_attrs, strerror(errno));
    return (-1);
  }

  memcpy(io->revsh_termios_attrs, io->saved_termios_attrs,
  ...
  if (tcsetattr(STDIN_FILENO, TCSANOW, io->revsh_termios_attrs) == -1) {
    report_error("do_control(): tcsetattr(STDIN_FILENO, TCSANOW, %lx): %s",
                 (unsigned long)io->revsh_termios_attrs, strerror(errno));
    return (-1);
  }
  ...
  if ((rc_fd = open(rc_file_exp.we_wordv[0], O_RDONLY)) != -1) {
    ...
    while ((io_bytes = read(rc_fd, message->data, io->message_data_size))) {
      if (io_bytes == -1) {
        report_error("do_control(): read(%d, %lx, %d): %s", rc_fd,
  ...
  if ((fcntl_flags = fcntl(io->local_in_fd, F_GETFL, 0)) == -1) {
    report_error("do_control(): fcntl(%d, F_GETFL, 0): %s", io->local_in_fd,
                 strerror(errno));
    return (-1);
  }
  ...
  if (fcntl(io->local_in_fd, F_SETFL, fcntl_flags) == -1) {
    report_error("do_control(): fcntl(%d, F_SETFL, %d): %s", io->local_in_fd,
                 fcntl_flags, strerror(errno));
    return (-1);
  }

  if ((fcntl_flags = fcntl(io->local_out_fd, F_GETFL, 0)) == -1) {
    report_error("do_control(): fcntl(%d, F_GETFL, 0): %s", io->local_out_fd,
                 strerror(errno));
    return (-1);
  }
  ...
  if (fcntl(io->local_out_fd, F_SETFL, fcntl_flags) == -1) {
    report_error("do_control(): fcntl(%d, F_SETFL, %d): %s", io->local_out_fd,
                 fcntl_flags, strerror(errno));
    return (-1);
  }
  ...
  if (retval == -1 && !io->eof) {
    if (errno == ECONNRESET) {
      ...
      report_error("do_control(): broker(%lx): %s", (unsigned long)config,
                   strerror(errno));
    }
  }
