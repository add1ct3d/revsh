 ...
 * report_error()
 *
 * Input: A pointer to our io_helper object, and the fmt specification as you
 *would find in a normal printf
 *  statement.
 * Output: None. Error reporting is a best effort maneuver.
 *
 * Purpose: Provide a wrapper for error reporting that inspects the io state and
...
void report_error(char *fmt, ...) {
  ...
  if (retval < 0) {
    if (verbose) {
      fprintf(stderr, "report_error(): vsnprintf(%lx, %d, %lx, %lx): %s\n",
              (unsigned long)error_string, ERROR_BUFF_SIZE - 1,
  ...
  if (verbose > 1) {
    if (io && !io->target) {
      // the control node's tty is raw right now. Stick a \r in there for
      // legibility.
      fprintf(stderr, "\r");
    }
  ...
  // This may have been a case where we are handling errors before io was set
  // up. Exit now if that's the case.
  if (!io) {
    return;
  }
 ...
 *
 * Purpose: Print the message to the logs. Used by report_error() to log an
 *error. Used elsewhere when there isn't an
 *          error to log things like successful connections.
 *
 * Note: Variadic functions are tricky. This version only converts the variadic
 *args into a string then calls the
...
int report_log_string(char *error_string) {
  ...
  if (io->log_stream) {
    ...
    if (fprintf(io->log_stream, "%s\n", error_string) < 0) {
      if (verbose) {
        fprintf(stderr,
                "report_log_string(): fprintf(%lx, \"%%s\\n\", %lx): %s\n",
                (unsigned long)io->log_stream, (unsigned long)error_string,
                strerror(errno));
      }
      return (-1);
    }
