...
struct proxy_node *proxy_node_new(char *proxy_string, int proxy_type) {
  ...
  if ((proxy_type == PROXY_DYNAMIC && !(count == 0 || count == 1)) ||
    ...
    report_error(
        "proxy_node_new(): Improper port forward syntax for proxy type '%d': "
        "%s",
        proxy_type, proxy_string);
    return (NULL);
  }
  ...
  if ((new_node->orig_request =
           (char *)calloc(strlen(proxy_string) + 1, sizeof(char))) == NULL) {
    report_error("proxy_node_new(): calloc(%d, sizeof(char)): %s",
                 (int)strlen(proxy_string), strerror(errno));
    free(new_node);
    return (NULL);
  }
  ...
  // Proxy strings can have a lot of different formats. This block is where we
  ...
  if (proxy_type == PROXY_DYNAMIC) {
    ...
    if (!second || (third = strchr(second, ':')) == NULL) {
      report_error("proxy_node_new(): Malformed proxy string: %s",
                   proxy_string);
      goto CLEANUP;
    }
  ...
  if (proxy_listen(new_node) == -1) {
    report_error(
        "proxy_node_new(): proxy_listen() failed. Skipping this proxy.");
    goto CLEANUP;
  }
...
int proxy_listen(struct proxy_node *cur_proxy_node) {
  ...
  if ((rv = getaddrinfo(cur_proxy_node->lhost, cur_proxy_node->lport, &hints,
                        &ai)) != 0) {
    report_error("proxy_listen(): getaddrinfo(%s, %s, %lx, %lx): %s",
                 cur_proxy_node->lhost, cur_proxy_node->lport,
                 (unsigned long)&hints, (unsigned long)&ai, gai_strerror(rv));
    return (-1);
  }
  ...
  if (p == NULL) {
    report_error("proxy_listen(): Failed to bind() to %s:%s",
                 cur_proxy_node->lhost, cur_proxy_node->lport);
    return (-1);
  }
  freeaddrinfo(ai);
  ...
  if (listen(listener, 10) == -1) {
    report_error("proxy_listen(): listen(%d, 10): %s", listener,
                 strerror(errno));
    return (-1);
  }
...
int proxy_connect(char *rhost_rport) {
  ...
  if ((rhost = (char *)calloc(count + 1, sizeof(char))) == NULL) {
    report_error("proxy_connect(): calloc(%d, %d): %s", count + 1,
  ...
  *(rport++) = '\0';
  ...
  if ((rv = getaddrinfo(rhost, rport, &hints, &ai)) != 0) {
    report_error("proxy_connect(): getaddrinfo(%s, %s, %lx, %lx): %s", rhost,
                 rport, (unsigned long)&hints, (unsigned long)&ai,
                 gai_strerror(rv));
    free(rhost);
    return (-2);
  }
 ...
 *
 * Purpose: Initialize a new proxy node, and put it into it's place in the
 *linked list.
 *
 ******************************************************************************/
struct proxy_node *proxy_node_create() {
  ...
  if ((cur_proxy_node =
           (struct proxy_node *)calloc(1, sizeof(struct proxy_node))) == NULL) {
    report_error("proxy_node_create(): calloc(1, %d): %s",
 ...
 *
 * Purpose: Initialize a new connection node, and put it into it's place in the
 *linked list.
 *
 ******************************************************************************/
struct connection_node *connection_node_create() {
  ...
  if ((cur_connection_node = (struct connection_node *)calloc(
           1, sizeof(struct connection_node))) == NULL) {
    report_error("connection_node_create(): calloc(1, %d): %s",
                 (int)sizeof(struct connection_node), strerror(errno));
    return (NULL);
  }
 ...
 *
 * Inputs: The tuple of (origin, id) that represents the unique id of a
 *connection.
 *
...
struct connection_node *connection_node_find(unsigned short origin,
  ...
  while (tmp_connection_node) {
    if ((tmp_connection_node->origin == origin) &&
        (tmp_connection_node->id == id)) {
...
int parse_socks_request(struct connection_node *cur_connection_node) {
  ...
  if (head[index] == 4) {
     ...
     *
     *  1 + 1 + 2 + 4 + 1
     *  = 9  Minimum number of bytes to ensure we have at least 1 char of
     * USERID.
     */
    ...
    if (head[index] != 1) {
      report_error(
          "parse_socks_request(): Socks 4: Unsupported CD. Only CD \"Connect\" "
          "is supported.");
      return (-1);
    }
    index += 1;
    ...
    if (head[index]) {
      report_error(
          "parse_socks_request(): Socks 4: USERID found, but not supported by "
          "this implementation.");
      return (-1);
    }

    // Socks 4a. Lets step through and grab the "domain".
    // Note: In the modern era, most clients use socks 4a. Even if the request
    // is a normal ipv4
    // address and socks 4 would work fine... instead, they use socks 4a and
    // send the ipv4 address
    // represented as a string. So here the "domain" may be something we pass to
    // dns to resolve.
    // It *might* just be an ascii string representation of an ip address (v4 or
    // v6).
    index++;
    if (!dst_addr_ptr[0] && !dst_addr_ptr[1] && !dst_addr_ptr[2] &&
        dst_addr_ptr[3]) {
      if (!(index < size)) {
        return (CON_SOCKS_INIT);
      }
      ...
      atype = 0x03;
      if ((cur_connection_node->rhost_rport = addr_to_string(
               atype, domain_name, dst_port_ptr, strlen(domain_name))) ==
          NULL) {
        report_error(
            "parse_socks_request(): addr_to_string(%d, %lx, %lx, %d): %s",
            atype, (unsigned long)dst_addr_ptr, (unsigned long)dst_port_ptr,
            (int)strlen(domain_name), strerror(errno));
        return (-1);
      }
    ...
    if ((cur_connection_node->rhost_rport =
             addr_to_string(atype, dst_addr_ptr, dst_port_ptr, 0)) == NULL) {
      report_error("parse_socks_request(): addr_to_string(%d, %lx, %lx, 0): %s",
                   atype, (unsigned long)dst_addr_ptr,
                   (unsigned long)dst_port_ptr, strerror(errno));
      return (-1);
    }
    ...
    if (cur_connection_node->state == CON_SOCKS_INIT) {
      ...
      for (i = 0; i < nmethods; i++) {
        ...
        // Someday we might implement more than the "NO AUTHENTICATION REQUIRED"
        // method...
        if (head[index] == 0x00) {
          noauth_found = 1;
        }
      ...
      if (noauth_found) {
        ...
        report_error(
            "parse_socks_request(): Socks 5: No supported auth type found. "
       ...
       * +----+-----+-------+------+----------+----------+
       *  (First byte of "Variable" is the strlen of the string that follows in
       * the DOMAINNAME case. No '\0' terminator.)
       *
       *  1 + 1 + 1 + 1 + 1 + V + 2
       *  = 7 + V...  So 7 is the minimum number of bytes before we can do
      ...
      if (head[index] != 1) {
        report_error(
            "parse_socks_request(): Socks 5: Unsupported CMD. Only CMD "
      ...
      if (atype == 0x01) {
        len = 4;
        // From the diagram above. 4 + Variable + 2, where Variable is 4 in the
        // ipv4 case.
        if (size < (4 + len + 2)) {
          return (CON_SOCKS_INIT);
        }
        ...
        // From the diagram above. 4 + Variable + 2, where Variable length is
        // defined in the first octet.
        if (size < (4 + 1 + len + 2)) {
          return (CON_SOCKS_INIT);
        }
        ...
        // From the diagram above. 4 + Variable + 2, where Variable is 16 in the
        // ipv6 case.
        if (size < (4 + len + 2)) {
          return (CON_SOCKS_INIT);
        }
      ...
      if ((cur_connection_node->rhost_rport = addr_to_string(
               atype, dst_addr_ptr, dst_port_ptr, len)) == NULL) {
        report_error(
            "parse_socks_request(): addr_to_string(%d, %lx, %lx, 0): %s", atype,
            (unsigned long)dst_addr_ptr, (unsigned long)dst_port_ptr,
            strerror(errno));
        return (-1);
      }
...
char *addr_to_string(int atype, char *addr, char *port, int len) {
  ...
  int string_len = 21;
  // ntohs returns a uint16_t. Max size of that number is 6 bytes. Plus one for
  // the leading ':'.
  // Note that the largest value *should* be 65535, which is 6 not 7 characters
  // long. Doing so will make
  // the compiler grumpy.
  int port_len = 7;
  ...
  if ((ptr = (char *)calloc(string_len + 1, sizeof(char))) == NULL) {
    report_error("addr_to_string(): calloc(%d, %d): %s", string_len + 1,
                 (int)sizeof(char), strerror(errno));
    return (NULL);
  }
  ...
  if (atype == 0x03) {
    ...
    snprintf(ptr, string_len,
             ...
             (unsigned char)addr[8], (unsigned char)addr[9],
             (unsigned char)addr[10], (unsigned char)addr[11],
             (unsigned char)addr[12], (unsigned char)addr[13],
             (unsigned char)addr[14], (unsigned char)addr[15], port_num);
  } else {
    snprintf(ptr, string_len, "%d.%d.%d.%d:%d", (unsigned char)addr[0],
             (unsigned char)addr[1], (unsigned char)addr[2],
             (unsigned char)addr[3], port_num);
  }
