...
int message_push() {
  ...
  if (message->data_type == DT_PROXY || message->data_type == DT_CONNECTION) {
    header_len += sizeof(message->header_type) +
                  sizeof(message->header_origin) + sizeof(message->header_id);
  ...
  if (io->remote_write(&tmp_short, sizeof(tmp_short)) == -1) {
    report_error("message_push(): remote_write(%lx, %d): %s",
                 (unsigned long)&tmp_short, (int)sizeof(tmp_short),
                 strerror(errno));
    return (-1);
  }
  ...
  if (io->remote_write(&message->data_type, sizeof(message->data_type)) == -1) {
    report_error("message_push(): remote_write(%lx, %d): %s",
                 (unsigned long)&message->data_type,
                 (int)sizeof(message->data_type), strerror(errno));
    return (-1);
  }
  ...
  if (io->remote_write(message->data, message->data_len) == -1) {
    report_error("message_push(): remote_write(%lx, %d): %s",
                 (unsigned long)message->data, message->data_len,
                 strerror(errno));
    return (-1);
  }
...
int message_pull() {
  ...
  if ((retval = io->remote_read(&header_len, sizeof(header_len))) == -1) {
    /* During a normal disconnect condition, this is where the message_pull
     * should fail, so check for EOF. */
    if (!io->eof) {
      report_error("message_pull(): remote_read(%lx, %d): %s",
                   (unsigned long)&header_len, (int)sizeof(header_len),
                   strerror(errno));
    }
    return (-1);
  }
  ...
  if ((retval = io->remote_read(&message->data_type,
                                sizeof(message->data_type))) == -1) {
    report_error("message_pull(): remote_read(%lx, %d): %s",
                 (unsigned long)&message->data_type,
                 (int)sizeof(message->data_type), strerror(errno));
    return (-1);
  ...
  if ((retval = io->remote_read(&message->data_len,
                                sizeof(message->data_len))) == -1) {
    report_error("message_pull(): remote_read(%lx, %d): %s",
                 (unsigned long)&message->data_len,
                 (int)sizeof(message->data_len), strerror(errno));
    return (-1);
  }
  ...
  if (message->data_type == DT_PROXY || message->data_type == DT_CONNECTION) {
    if ((retval = io->remote_read(&message->header_type,
                                  sizeof(message->header_type))) == -1) {
      report_error("message_pull(): remote_read(%lx, %d): %s",
                   (unsigned long)&message->header_type,
                   (int)sizeof(message->header_type), strerror(errno));
      return (-1);
    }
    message->header_type = ntohs(message->header_type);
    header_len -= sizeof(message->header_type);

    if ((retval = io->remote_read(&message->header_origin,
                                  sizeof(message->header_origin))) == -1) {
      report_error("message_pull(): remote_read(%lx, %d): %s",
                   (unsigned long)&message->header_origin,
                   (int)sizeof(message->header_origin), strerror(errno));
      return (-1);
    }
    message->header_origin = ntohs(message->header_origin);
    header_len -= sizeof(message->header_origin);

    if ((retval = io->remote_read(&message->header_id,
                                  sizeof(message->header_id))) == -1) {
      report_error("message_pull(): remote_read(%lx, %d): %s",
                   (unsigned long)&message->header_id,
                   (int)sizeof(message->header_id), strerror(errno));
      return (-1);
    }
    ...
    if (message->header_type == DT_PROXY_HT_CREATE ||
        message->header_type == DT_PROXY_HT_REPORT ||
        message->header_type == DT_CONNECTION_HT_CREATE) {
      if ((retval = io->remote_read(&message->header_proxy_type,
                                    sizeof(message->header_proxy_type))) ==
          -1) {
        report_error("message_pull(): remote_read(%lx, %d): %s",
                     (unsigned long)&message->header_proxy_type,
                     (int)sizeof(message->header_proxy_type), strerror(errno));
        return (-1);
      }
  ...
  /* Ignore any remaining header data as unknown, and probably from a more
  ...
  if (header_len) {
    ...
    if ((retval = io->remote_read(message->data, header_len)) == -1) {
      report_error("message_pull(): remote_read(%lx, %d): %s",
                   (unsigned long)message->data, header_len, strerror(errno));
      return (-1);
    }
  }
 ...
 *
 * Purpose: Make a new message_helper node and fill it with data. Probably for
 *the write buffering case where a write()
...
struct message_helper *message_helper_create(char *data,
  ...
  if (!new_mh) {
    report_error("message_helper_create(): calloc(1, %d): %s",
                 (int)sizeof(struct message_helper), strerror(errno));
    return (NULL);
  }
