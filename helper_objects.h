...
struct proxy_node {
  ...
  // The strings representing this proxy may be used later on for error
  // reporting.
  char *lhost;
  char *lport;
  char *rhost_rport;
...
struct connection_node {
  ...
  // A copy of the original rhost_rport string in the related proxy_node struct,
  // to simplify retry requests.
  // Note, this has to be a copy, because in the remote connection state, the
  // original proxy node does not exist.
  char *rhost_rport;
  ...
  //  Note, no write_tail element. Iterate through every time you want to add an
  //  element, thus calculating the message depth dynamically.
  //  If MAX_MESSAGE_DEPTH is hit, tell the remote node to stop listening its
  //  associated fd.
 ...
 ******************************************************************************/
struct io_helper {
  // This variable is used in the keep-alive modes, to ensure we don't try to
  // close STDIN/STDOUT/STDERR again.
  int first_run;
  ...
  // Done as a "target" flag, so control is always 0. If we expand to allow node
  // chaining in the future,
  // then each target node will have a non-zero target number. This is for
  // future expansion.
  int target;
  int child_sid;

  /* We use pointers to functions here so we can invoke the appropriate function
   * on the backend (crypto / no crypto). */
  ...
  // This is the match for the config->interactive, but represents the actual
  // state of interaction for this run.
  // Useful in the keepalive case.
  int interactive;

  // Save the original tty settings here for inspection during runtime and later
  // restoration.
  ...
  // this message_helper node is used internally by the io_helper for the
  // processing of the message bus.
  struct message_helper message;

  // flag for EOF condition.
  int eof;

  // Flag representing that the initialization process has completed. Used in
  // report_error() to determine
  // if it is ok to leverage the message bus for error reporting.
  int init_complete;

  // this message_helper node is used for the write buffer queue for the
  // tty/shell.
