 ...
 *	in_the_key_of_c
 *
 *	emptymonkey's tool for converting RSA key pairs into C source code
 *representations.
 *
 *	2014-10-12
 *
...
void usage() {
  ...
  fprintf(
      stderr,
      "\t-k\tPrint the C code equivalent of the RSA KEY_FILE in DER format.\n");
  fprintf(
      stderr,
      "\t-c\tPrint the C code equivalent of RSA CERT_FILE in DER format.\n");
  fprintf(stderr,
          "\t-f\tPrint the C code equivalent of the SHA1 fingerprint for "
          "CERT_FILE.\n");
  fprintf(stderr,
          "\n\tNote: At least one of the two options, -k or -c, must be "
          "present.\n");
  fprintf(stderr, "\n");
  exit(-1);
}
...
int main(int argc, char **argv) {
  ...
  /* OpenSSL isn't very good at checking buffer sizes or telling you how big
   * they should be. */
  /* We will grab a pages worth and cross our fingers. /sigh */
  buffer_len = getpagesize();
  if ((buffer_head = (unsigned char *)calloc(buffer_len, sizeof(char))) ==
      NULL) {
  ...
  if (key_path) {
    ...
    if ((key_short_name =
             (char *)calloc(strlen(tmp_char_ptr) + 1, sizeof(char))) == NULL) {
      fprintf(stderr, "%s: calloc(%d, %d): %s\n", program_invocation_short_name,
    ...
    if (!(key_len = i2d_RSAPrivateKey(key, &buffer_ptr))) {
      fprintf(stderr, "%s: i2d_RSAPrivateKey(%lx, %lx): %s\n",
              program_invocation_short_name, (unsigned long)key,
              (unsigned long)buffer_ptr, strerror(errno));
      ERR_print_errors_fp(stderr);
      exit(-1);
    }
  ...
  if (cert_path) {
    ...
    if ((cert_stream = fopen(cert_path, "r")) == NULL) {
      fprintf(stderr, "%s: fopen(%s, \"r\"): %s\n",
              program_invocation_short_name, cert_path, strerror(errno));
      exit(-1);
    }

    if ((cert = PEM_read_X509(cert_stream, NULL, NULL, NULL)) == NULL) {
      fprintf(stderr, "%s: PEM_read_X509(%lx, NULL, NULL, NULL): %s\n",
              program_invocation_short_name, (unsigned long)cert_stream,
              strerror(errno));
      ERR_print_errors_fp(stderr);
      exit(-1);
    }
    ...
    if (do_fingerprint) {
      ...
      if (!X509_digest(cert, digest, fingerprint, &fingerprint_len)) {
        fprintf(stderr, "%s: !X509_digest(%lx, %lx, %lx, %lx): %s\n",
                program_invocation_short_name, (unsigned long)cert,
                (unsigned long)digest, (unsigned long)fingerprint,
                (unsigned long)&fingerprint_len, strerror(errno));
        ERR_print_errors_fp(stderr);
        exit(-1);
      }
      ...
      /* For some reason, openssl outputs C code as uppercase hex letters, but
       * fingerprints are lowercase. ?? */
