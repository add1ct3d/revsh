 ...
 *  The broker loop was more complicated, but still within the understanding of
 * man.
 *  The kiddies then cried out "Where be our tunnels?! How canz we be 1337 w/out
 * crypto tunnels?!! CAN HAS SOCKS?!"
 *  The BOFH heard the wailing, lamenting, and gnashing of kiddie teeths and
 * added socks proxy support to the broker loop.
 *  It worked and the kiddies were happy, but now the loop was complicated
 * beyond the understanding of man.
 *  It was in this age that the BOFA moved the inner workings of the broker loop
 * here, with a handler for each case.
...
int handle_signal_sigwinch() {
  ...
  if ((retval = ioctl(io->local_out_fd, TIOCGWINSZ, io->tty_winsize)) == -1) {
    report_error("handle_signal_sigwinch(): ioctl(%d, TIOCGWINSZ, %lx): %s",
  ...
  *((unsigned short *)(message->data + message->data_len)) =
      htons(io->tty_winsize->ws_col);
  message->data_len += sizeof(io->tty_winsize->ws_col);

  if ((retval = message_push()) == -1) {
    report_error("handle_signal_sigwinch(): message_push(): %s",
                 strerror(errno));
    return (-1);
  }
  ...
  while (io->tty_write_head) {
    ...
    if (retval == -1) {
      if (errno != EINTR && errno != EAGAIN && errno != EWOULDBLOCK) {
        report_error("handle_local_write(): write(%d, %lx, %d): %s",
                     io->local_out_fd, (unsigned long)tmp_message->data,
                     tmp_message->data_len, strerror(errno));
        return (-1);
      }
    }
    ...
    if (retval != tmp_message->data_len) {
      ...
      memmove(tmp_message->data, tmp_message->data + retval,
              tmp_message->data_len);
      break;
    }
...
int handle_local_read() {
  ...
  if ((retval = read(io->local_in_fd, message->data, io->message_data_size)) ==
      -1) {
    if (errno != EINTR) {
      if (errno == EIO) {
        return (-2);
      } else {
        report_error("handle_local_read(): read(%d, %lx, %d): %s",
                     io->local_in_fd, (unsigned long)message->data,
                     io->message_data_size, strerror(errno));
        return (retval);
      }
    }
    ...
    if (message->data_len) {
      if (!io->target && io->interactive) {
        if ((retval = escape_check()) < 0) {
          if (retval == -1) {
            report_error("handle_local_read(): escape_check(): %s",
                         strerror(errno));
          }
          return (retval);
        }
      ...
      // Check again for data_len, because we may have consumed the characters
      // in the buffer during the escape_check().
      if (message->data_len) {
        io->tty_io_written += message->data_len;
        if ((retval = message_push()) == -1) {
          report_error("handle_local_read(): message_push(): %s",
                       strerror(errno));
          return (-1);
        }
      }
...
int handle_message_dt_tty() {
  ...
  if (retval == -1) {
    if (errno != EINTR && errno != EAGAIN && errno != EWOULDBLOCK) {
      report_error("handle_message_dt_tty(): write(%d, %lx, %d): %s",
                   io->local_out_fd, (unsigned long)message->data,
                   message->data_len, strerror(errno));
      return (-1);
    }
  }

  if (retval != message->data_len) {
    new_message = message_helper_create(message->data + retval,
                                        message->data_len - retval,
                                        io->message_data_size);

    if (!new_message) {
      report_error(
          "handle_message_dt_tty(): message_helper_create(%lx, %d, %d): %s\n",
          (unsigned long)message->data + retval, message->data_len - retval,
          io->message_data_size, strerror(errno));
      return (-1);
    }
...
int handle_message_dt_winresize() {
  ...
  if (message->data_len !=
      sizeof(io->tty_winsize->ws_row) + sizeof(io->tty_winsize->ws_col)) {
    report_error(
        "handle_message_dt_winresize(): DT_WINRESIZE termios: not enough "
        "data!");
    return (-1);
  }

  io->tty_winsize->ws_row = ntohs(*((unsigned short *)message->data));
  io->tty_winsize->ws_col =
      ntohs(*((unsigned short *)(message->data + sizeof(unsigned short))));

  if ((retval = ioctl(io->local_out_fd, TIOCSWINSZ, io->tty_winsize)) == -1) {
    report_error("handle_message_dt_winresize(): ioctl(%d, %d, %lx): %s",
  ...
  if ((retval = kill(-(io->child_sid), SIGWINCH)) == -1) {
    report_error("handle_message_dt_winresize(): kill(%d, SIGWINCH): %s",
                 -(io->child_sid), strerror(errno));
    return (-1);
  }
...
int handle_message_dt_proxy_ht_destroy() {
  ...
  if ((cur_proxy_node =
           proxy_node_find(message->header_origin, message->header_id))) {
    if (verbose && header_errno) {
      fprintf(
          stderr,
          "\rhandle_message_dt_proxy_ht_destroy(): Connection %s closed: %s\n",
...
int handle_message_dt_proxy_ht_create() {
  ...
  // This should be non-fatal. Common case is unable to bind() because port is
  // already in use.
  if ((cur_proxy_node =
           proxy_node_new(message->data, message->header_proxy_type)) == NULL) {
    report_error(
        "handle_message_dt_proxy_ht_create(): proxy_node_new(%lx, %d): %s",
        (unsigned long)message->data, message->header_proxy_type,
        strerror(errno));
    return (-2);
  }
  ...
  if (io->target) {
    if (handle_send_dt_proxy_ht_report(cur_proxy_node) == -1) {
      report_error(
          "handle_message_dt_proxy_ht_create(): "
          "handle_send_dt_proxy_ht_report(%lx): %s",
          (unsigned long)cur_proxy_node, strerror(errno));
      return (-1);
    }
  }
  return (0);
...
int handle_message_dt_proxy_ht_report() {
  ...
  if ((cur_proxy_node->orig_request =
           (char *)calloc(message->data_len + 1, sizeof(char))) == NULL) {
    report_error("handle_message_dt_proxy_ht_report(): calloc(%d, %d): %s",
                 message->data_len + 1, (int)sizeof(char), strerror(errno));
    return (-1);
  }
...
int handle_message_dt_connection_ht_create() {
  ...
  if (message->header_proxy_type == PROXY_TUN ||
    ...
    if ((retval = handle_message_dt_connection_ht_create_tun_tap()) == -1) {
      report_error(
          "handle_message_dt_connection_ht_create(): "
          "handle_message_dt_connection_ht_create_tun_tap(): %s",
          strerror(errno));
    }
    ...
    if (retval == -2) {
      if (verbose > 2) {
        fprintf(stderr,
                "\rhandle_message_dt_connection_ht_create(): Unable to create "
                "tun/tap interface.\n");
      }
      if (handle_send_dt_connection_ht_destroy(
              message->header_origin, message->header_id, ENODEV) == -1) {
        report_error(
            "handle_message_dt_connection_ht_create(): "
            "handle_send_dt_connection_ht_destroy(%d, %d, ENODEV): %s",
            message->header_origin, message->header_id, strerror(errno));
        return (-1);
      }
    }
    return (retval);
  ...
  if ((cur_connection_node = connection_node_create()) == NULL) {
    report_error(
        "handle_message_dt_connection_ht_create(): connection_node_create(): "
        "%s",
        strerror(errno));
    return (-1);
  }
  ...
  if ((cur_connection_node->rhost_rport =
           (char *)calloc(message->data_len + 1, sizeof(char))) == NULL) {
    report_error("handle_message_dt_connection_ht_create(): calloc(%d, %d): %s",
                 message->data_len + 1, (int)sizeof(char), strerror(errno));
    return (-1);
  }
  ...
  if ((cur_connection_node->fd =
           proxy_connect(cur_connection_node->rhost_rport)) < 0) {
    if (cur_connection_node->fd == -1) {
      if (verbose) {
        report_error(
            "handle_message_dt_connection_ht_create(): proxy_connect(\"%s\"): "
            "%s\n",
    ...
    if (handle_send_dt_connection_ht_destroy(cur_connection_node->origin,
                                             cur_connection_node->id,
                                             errno) == -1) {
      report_error(
          "handle_message_dt_connection_ht_create(): "
          "handle_send_dt_connection_ht_destroy(%d, %d, errno): %s",
...
#ifdef FREEBSD
  // ENOSYS : It's not clear to me if ENOSYS is reserved for the OS or if this
  // is a reasonable use case.
  handle_send_dt_connection_ht_destroy(origin, id, ENOSYS);
  report_error(
      "handle_message_dt_connection_ht_create_tun_tap(): revsh does not "
      "currently support tun/tap devices on FreeBSD.");
  return (-2);
#else
...
int handle_connection_activate(struct connection_node *cur_connection_node) {
  ...
  if (getsockopt(cur_connection_node->fd, SOL_SOCKET, SO_ERROR, &optval,
                 &optlen) == -1) {
    if (handle_send_dt_connection_ht_destroy(cur_connection_node->origin,
                                             cur_connection_node->id,
                                             errno) == -1) {
  ...
  if (optval != 0) {
    ...
    if (handle_send_dt_connection_ht_destroy(
            cur_connection_node->origin, cur_connection_node->id, 0) == -1) {
      report_error(
          "handle_connection_activate(): "
          "handle_send_dt_connection_ht_destroy(%d, %d, 0): %s",
          cur_connection_node->origin, cur_connection_node->id,
          strerror(errno));
      return (-1);
    }
...
int handle_message_dt_connection_ht_active_dormant() {
  ...
  if ((cur_connection_node = connection_node_find(
           message->header_origin, message->header_id)) == NULL) {
    if (handle_send_dt_connection_ht_destroy(message->header_origin,
                                             message->header_id, 0) == -1) {
      report_error(
...
int handle_message_dt_connection_ht_data() {
  ...
  if (retval == -1) {
    if (errno != EINTR && errno != EAGAIN && errno != EWOULDBLOCK) {
      report_error("handle_message_dt_connection(): write(%d, %lx, %d): %s",
                   cur_connection_node->fd, (unsigned long)message->data,
                   message->data_len, strerror(errno));
  ...
  if (retval != message->data_len) {
    ...
    if (!new_message) {
      report_error(
          "handle_message_dt_connection(): message_helper_create(%lx, %d, %d): "
          "%s",
          (unsigned long)message->data + retval, message->data_len - retval,
          io->message_data_size, strerror(errno));
      return (-1);
    ...
    if (!cur_connection_node->write_head) {
      ...
      if (count == MESSAGE_DEPTH_MAX) {
        ...
        if ((retval = message_push()) == -1) {
          report_error("handle_message_dt_connection(): message_push(): %s",
                       strerror(errno));
          return (-1);
        }
      }
...
int handle_proxy_read(struct proxy_node *cur_proxy_node) {
  ...
  if ((cur_connection_node = connection_node_create()) == NULL) {
    report_error("handle_proxy_read(): connection_node_create(): %s",
                 strerror(errno));
    return (-1);
  }

  if ((cur_connection_node->fd = accept(cur_proxy_node->fd, NULL, NULL)) ==
      -1) {
    report_error("handle_proxy_read(): accept(%d, NULL, NULL): %s",
                 cur_proxy_node->fd, strerror(errno));
    return (-1);
  }
  ...
  if (cur_proxy_node->proxy_type == PROXY_DYNAMIC) {
    ...
    if ((cur_connection_node->buffer_head =
             (char *)calloc(SOCKS_REQ_MAX, sizeof(char))) == NULL) {
      report_error("handle_proxy_read(): calloc(%d, %d): %s\r", SOCKS_REQ_MAX,
    ...
    if ((cur_connection_node->rhost_rport =
             (char *)calloc(count + 1, sizeof(char))) == NULL) {
      report_error("handle_proxy_read(): calloc(%d, %d): %s", count + 1,
                   (int)sizeof(char), strerror(errno));
      return (-1);
    }
    memcpy(cur_connection_node->rhost_rport, cur_proxy_node->rhost_rport,
           count);
    ...
    if (handle_send_dt_connection_ht_create(cur_connection_node) == -1) {
      report_error(
          "handle_proxy_read(): handle_send_dt_connection_ht_create(%lx): %s",
          (unsigned long)cur_connection_node, strerror(errno));
      return (-1);
    }
  }
...
int handle_connection_write(struct connection_node *cur_connection_node) {
  ...
  while (cur_connection_node->write_head) {
    ...
    retval = write(cur_connection_node->fd, tmp_message->data,
                   tmp_message->data_len);
    ...
    if (!cur_connection_node->write_head) {
      ...
      if ((retval = message_push()) == -1) {
        report_error("handle_connection_write(): message_push(): %s",
                     strerror(errno));
        return (-1);
      }
    }
...
int handle_connection_read(struct connection_node *cur_connection_node) {
  ...
  errno = 0;
  if ((retval = read(cur_connection_node->fd, message->data,
                     io->message_data_size)) < 1) {
    if ((verbose > 2) && retval) {
    ...
    if (handle_send_dt_connection_ht_destroy(cur_connection_node->origin,
                                             ...
                                             errno) == -1) {
      report_error(
          "handle_connection_read(): handle_send_dt_connection_ht_destroy(%d, "
          "%d, errno): %s",
          cur_connection_node->origin, cur_connection_node->id,
          strerror(errno));
      return (-1);
  ...
  if ((retval = message_push()) == -1) {
    report_error("handle_connection_read(): message_push(): %s",
                 strerror(errno));
    return (-1);
  }
...
int handle_connection_socks_init(struct connection_node *cur_connection_node) {
  ...
  if ((retval = read(cur_connection_node->fd, cur_connection_node->buffer_tail,
                     cur_connection_node->buffer_size -
                         (cur_connection_node->buffer_tail -
                          cur_connection_node->buffer_head) -
                         1)) < 1) {
    // Sorry if this syntax feels awkward. The case defined below is the re-call
    // of the handle_connection_socks_init() by the
    // handle_connection_socks_init()
    // when CON_SOCKS_V5_AUTH has occured and the buffer may already be ready
    // for processing. It will generally fail the read and want to drop
    // though below for processing. We are if()'ing on the negation of that case
    // to handle when it's just a normal good ol'fashioned read()
    // failure.
    if (!((cur_connection_node->state == CON_SOCKS_V5_AUTH) &&
          (cur_connection_node->buffer_tail -
           cur_connection_node->buffer_ptr) &&
  ...
  if ((new_state = parse_socks_request(cur_connection_node)) == -1) {
    report_error(
        "handle_connection_socks_init(): parse_sock_request(%lx): Malformed "
        "SOCKS request.",
  ...
  if (new_state == CON_SOCKS_INIT) {
    ...
    if (!(cur_connection_node->buffer_size -
          (cur_connection_node->buffer_tail -
           cur_connection_node->buffer_head) -
          1)) {
      report_error(
  ...
  if (retval == -1) {
    report_error("handle_connection_socks_init(): write(%d, %lx, %d): %s",
                 cur_connection_node->fd, (unsigned long)reply_buff,
                 reply_buff_len, strerror(errno));
    return (-1);
  }

  if (retval != reply_buff_len) {
    report_error(
        "handle_connection_socks_init(): write(%d, %lx, %d): Unable to send "
        "SOCKS reply.",
        cur_connection_node->fd, (unsigned long)reply_buff, reply_buff_len);
    if (handle_send_dt_connection_ht_destroy(
            cur_connection_node->origin, cur_connection_node->id, 0) == -1) {
      report_error(
          "handle_connection_socks_init(): "
  ...
  // Handle the case where we have a rude client that doesn't wait for data and
  // just fills our buffer with both halves of the
  ...
  if ((cur_connection_node->state == CON_SOCKS_V5_AUTH) &&
    ...
    if ((retval = handle_connection_socks_init(cur_connection_node)) == -1) {
      report_error("broker(): handle_connection_socks_init(%lx): %s",
                   (unsigned long)cur_connection_node, strerror(errno));
      return (-1);
    }
    return (retval);
  ...
  if (cur_connection_node->state == CON_ACTIVE) {
    if (handle_send_dt_connection_ht_create(cur_connection_node) == -1) {
      report_error(
          "handle_connection_socks_init(): "
          "handle_send_dt_connection_ht_create(%lx): %s",
          (unsigned long)cur_connection_node, strerror(errno));
      return (-1);
    }
    ...
    if (cur_connection_node->buffer_ptr != cur_connection_node->buffer_tail) {
      ...
      memcpy(
          message->data, cur_connection_node->buffer_ptr,
          cur_connection_node->buffer_tail - cur_connection_node->buffer_ptr);
      message->data_len =
          cur_connection_node->buffer_tail - cur_connection_node->buffer_ptr;
      ...
      cur_connection_node->io_written += message->data_len;
      if ((retval = message_push()) == -1) {
        report_error("handle_connection_socks_init(): message_push(): %s",
                     strerror(errno));
        return (-1);
      }
    }
...
int handle_send_dt_proxy_ht_destroy(unsigned short origin, unsigned short id,
  ...
  if ((retval = message_push()) == -1) {
    report_error("handle_send_dt_proxy_ht_destroy(): message_push(): %s",
                 strerror(errno));
    return (-1);
  }
...
int handle_send_dt_proxy_ht_report(struct proxy_node *cur_proxy_node) {
  ...
  message->data_len =
      retval > io->message_data_size ? io->message_data_size : retval;
  memcpy(message->data, cur_proxy_node->orig_request, message->data_len);

  if ((retval = message_push()) == -1) {
    report_error("handle_send_dt_proxy_ht_report(): message_push(): %s",
                 strerror(errno));
    return (-1);
  }
...
int handle_send_dt_connection_ht_destroy(unsigned short origin,
  ...
  if ((retval = message_push()) == -1) {
    report_error("handle_send_dt_connection_ht_destroy(): message_push(): %s",
                 strerror(errno));
    return (-1);
  }
...
struct connection_node *handle_tun_tap_init(int ifr_flag) {
#ifdef FREEBSD
  report_error(
      "handle_tun_tap_init(): revsh does not currently support tun/tap devices "
      "on FreeBSD.");
  return (NULL);
#else
  ...
  if ((cur_connection_node = connection_node_create()) == NULL) {
    report_error("handle_tun_tap_init(): connection_node_create(): %s",
                 strerror(errno));
    return (NULL);
  }
  ...
  if ((cur_connection_node->fd = open(DEV_NET_TUN, O_RDWR)) == -1) {
    ...
    report_error("handle_tun_tap_init(): open(%s, O_RDWR): %s", DEV_NET_TUN,
                 strerror(errno));
    return (NULL);
  }
  cur_connection_node->origin = io->target;
  ...
  if (ioctl(cur_connection_node->fd, TUNSETIFF, (void *)&ifr) == -1) {
    report_error("handle_tun_tap_init(): ioctl(%d, TUNSETIFF, %lx): %s: %s",
                 cur_connection_node->fd, (unsigned long)((void *)&ifr),
                 ifr_flag_name, strerror(errno));
  ...
  if ((cur_connection_node->rhost_rport =
           (char *)calloc(count + 1, sizeof(char))) == NULL) {
    report_error("handle_tun_tap_init(): calloc(%d, %d): %s", count + 1,
                 (int)sizeof(char), strerror(errno));
    connection_node_delete(cur_connection_node);
    return (NULL);
  }
  ...
  /*
           Really Linux?! Are you fucking kidding me?? I can't perform the MTU
     set on the tun/tap fd directly??
           I have to open a *random unrelated socket* just to pass it's fd to
     the MTU ioctl() call, only to close it out
           immediately after?! Holy shit that's janky!!
   */
  if ((tmp_sock = socket(AF_INET, SOCK_DGRAM, 0)) == -1) {
    report_error("handle_tun_tap_init(): socket(AF_LOCAL, SOCK_DGRAM, 0): %s",
                 strerror(errno));
    connection_node_delete(cur_connection_node);
    return (NULL);
  }
  ...
  if (ioctl(tmp_sock, SIOCGIFMTU, (void *)&ifr) == -1) {
    report_error("handle_tun_tap_init(): ioctl(%d, SIOCGIFMTU, %lx): %s",
                 cur_connection_node->fd, (unsigned long)&ifr, strerror(errno));
    connection_node_delete(cur_connection_node);
    return (NULL);
  }

  /*
           If the mtu on the tun / tap device is larger than the message data
     buffer, reduce it.
           This ensures the we can always fit a full frame / packet inside one
     message.
           Given that the default is that the message data buffer is probably a
     page of memory, and that is probably 4k in size, this will
  ...
  if (ifr.ifr_mtu > io->message_data_size) {
    ...
    if (ioctl(tmp_sock, SIOCSIFMTU, (void *)&ifr) == -1) {
      report_error("handle_tun_tap_init(): ioctl(%d, SIOCSIFMTU, %lx): %s",
                   cur_connection_node->fd, (unsigned long)&ifr,
                   strerror(errno));
      connection_node_delete(cur_connection_node);
      return (NULL);
    }
